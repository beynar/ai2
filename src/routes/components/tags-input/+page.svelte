	...
	{#snippet examples()}
		...
		<ComponentCard description="Free mode — type and press Enter to add a tag">
			<div class="w-full max-w-md">
				...
				{#if value1?.length}
					<p class="text-foreground-muted mt-2 text-xs">Tags: {value1.join(', ')}</p>
				{/if}
			</div>
		</ComponentCard>
		...
		<ComponentCard description="Restricted to a static list, shows all options on focus">
			<div class="w-full max-w-md">
				...
				/>
				{#if value2?.length}
					<p class="text-foreground-muted mt-2 text-xs">Tags: {value2.join(', ')}</p>
				{/if}
			</div>
		</ComponentCard>
		...
		<ComponentCard description="Simulated API call with 300ms delay">
			<div class="w-full max-w-md">
				...
				{#if value3?.length}
					<p class="text-foreground-muted mt-2 text-xs">Tags: {value3.join(', ')}</p>
				{/if}
			</div>
		</ComponentCard>
		...
		<ComponentCard description="Restricted list, but also allows custom free-text tags">
			<div class="w-full max-w-md">
				...
				/>
				{#if value4?.length}
					<p class="text-foreground-muted mt-2 text-xs">Tags: {value4.join(', ')}</p>
				{/if}
			</div>
		</ComponentCard>
		...
		<ComponentCard description="Limited to a maximum of 3 tags">
			<div class="w-full max-w-md">
				...
				{#if value5?.length}
					<p class="text-foreground-muted mt-2 text-xs">Tags: {value5.join(', ')}</p>
				{/if}
			</div>
		</ComponentCard>
