<script lang="ts">
	const guides = [
		...
		},
		{
			...
			body: 'Drag, keyboard mode, both resize edges, API updates, paste, and history share one immutable mutation boundary. Focus an empty slot and press Space, extend with arrows, then press Enter to create a keyboard range; pointer users can click its two endpoints. Structural/range checks run before resource hours, overlap, and custom policy. onItemUpdate may reject or adjust, and adjustments are revalidated. onItemsChange receives one guarded transaction. Mod+C/Mod+V copies a selected occurrence as a standalone item; Mod+Z and Mod+Shift+Z undo and redo up to historyLimit. Empty-slot creation ends at onSlotSelect, whose source identifies drag-create, keyboard, or single-pointer input: compose a Svelai Dialog and Form, then add the validated domain item yourself.'
		},
		{
			title: 'Snippets and theme',
			body: 'header exposes ready-made navigation, title, view, date-picker, and action snippets. item customizes event content in every view, including agenda, and receives defaultContent plus markerContent, titleContent, and timeContent. overflowContent can wrap defaultContent without rebuilding the draggable, accessible hidden-item list. itemTooltip customizes the item HoverCard and, with timeGutter, empty, and loadingContent, exposes defaultContent. dayHeader, monthCell, allDay, overflow, agendaDetails, resourceHeader, nowIndicatorContent, and dragPreview retain their focused contracts. Semantic wrappers, focus, disclosures, drag wiring, and announcements remain calendar-owned; actions and agendaDetails stay additive. Override theme per instance or use setEventCalendarTheme globally; CSS metric variables tune geometry.'
		},
		{
