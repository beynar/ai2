...
- **density**: 'small' | 'normal' | 'large' (default: 'normal') - Spacing token scaling header, list and card paddings/gaps; also stamped as data-density on the root
...
- **card**: Snippet<[{ card, column, index, columnIndex, isDragging, handle }]> - Custom card renderer (default: title + muted description). \`index\` and \`columnIndex\` are the rendered positions (following the live preview unless \`indicator\` is enabled); \`isDragging\` identifies the dragged card; \`handle\` is the ready-made drag grip (data-dnd-handle) — render it wherever the grip belongs, it is empty unless \`cardHandle\` is on
- **columnHeader**: Snippet<[{ column, index }]> - Custom header (default: color dot + title + count, count shows "n / limit" when a limit is set)
- **columnFooter**: Snippet<[{ column, index }]> - Rendered below the card list (e.g. an "Add card" button), via the \`footer\` theme part
- **column**: Snippet<[{ column, index, header, items, footer }]> - Full column composition: \`header\`/\`items\`/\`footer\` are ready-made snippets (\`items\` is the dnd-wired card list — drop target, live preview, animations). Render them in any arrangement with your own chrome; replaces the default layout inside the column shell.
...
\`\`\`
- **empty**: Snippet<[{ column }]> - Rendered inside an empty column's list (via the \`empty\` theme part). Nothing renders by default — the empty list keeps its min-height and data-dnd-over tint as the drop area
