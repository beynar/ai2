...
export type KanbanProps<C extends KanbanCard = KanbanCard> = WithAttachments<
	HTMLAttributes<HTMLDivElement> & {
		...
		/** Called once per completed card move (same-column reorders included), after `columns` was reassigned — `detail.columns` is the new state. */
		 ...
		 * drop. The bound state only changes at drop. Disabled automatically under
