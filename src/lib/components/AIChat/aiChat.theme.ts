...
const defaultRoot = cva({
	base: 'flex h-full min-h-0 min-w-0 flex-col overflow-hidden rounded-lg border border-background-muted bg-background'
});
const defaultHeader = cva({ base: 'shrink-0 border-b border-background-muted px-4 py-3' });
...
const defaultSuggestions = cva({ base: 'border-t border-background-muted px-3 py-2' });
const defaultComposer = cva({ base: 'shrink-0 border-t border-background-muted p-3' });
const defaultControls = cva({ base: 'flex min-w-0 items-center gap-2' });
const defaultFooter = cva({ base: 'shrink-0 border-t border-background-muted px-4 py-2' });
