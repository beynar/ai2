...
const defaultGroupAction = cva({
	base: 'text-foreground hover:bg-background-dark hover:text-foreground absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-none transition group-data-[collapsible=icon]:hidden focus-visible:ring-2 focus-visible:ring-primary/40 [&>svg]:size-4 [&>svg]:shrink-0'
});
...
const defaultSubMenu = cva({
	base: 'border-background-dark mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5 group-data-[collapsible=icon]:hidden'
});
...
const defaultSearch = cva({
	base: 'border-background-dark bg-background-light text-foreground placeholder:text-foreground/45 h-8 w-full rounded-md border px-8 text-sm shadow-none outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/20'
});
...
const defaultSeparator = cva({
	base: 'bg-background-dark mx-2 h-px w-auto shrink-0'
});
...
const defaultAvatar = cva({
	base: 'bg-background-dark text-foreground flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-md text-xs font-medium'
});
