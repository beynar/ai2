<script lang="ts" module>
	...
	import { fileInputTheme } from '$lib/components/Form/File/fileInput.js';
	export const setFileInputTheme = setComponentTheme<typeof fileInputTheme>('fileInput');
...
<script lang="ts" generics="Mode extends 'single' | 'multiple' = 'single'">
	...
	import type { FileInputProps, FileInputType, FileInputValue } from './fileInput.js';
	import { slide } from '../../../transitions/transition.js';
...
<div use:dropzone.zone>
	<Field
		...
		theme={{
			...
			inputContainer: {
				...
				base: classes.inputContainer({ class: theme?.inputContainer?.base, state: dropzone.state })
			}
		}}
		attrs={{
	...
	>
		...
		>
			{#each dropzone.files as fil, i (fil.name)}
				{@const size = dropzone.formatSize(fil.size)}
				<div transition:slide={{ duration: 300, opacity: 0.9, scale: 0.9 }}>
					<Slot
						payload={{
							file: fil,
							index: i,
					...
					>
						{#if isImage(fil)}
							<img
								src={getImagePreview(fil)}
								alt={fil.name}
								class="border-surface-muted h-16 w-16 shrink-0 rounded border object-cover"
							/>
						{/if}
						<div class="ui-file-input-file-info min-w-0 flex-1">
							<div class="ui-file-input-file-name truncate">{fil.name}</div>
							<div class="ui-file-input-file-size text-contrast-muted text-xs">{size}</div>
						</div>
