...
const defaultRoot = cva({ base: 'not-prose mb-4 min-w-0' });
const defaultTrigger = cva({
	base: 'flex w-fit max-w-full min-w-0 items-center justify-start gap-2 text-sm text-foreground/65 transition-colors hover:text-foreground'
});
...
const defaultContent = cva({
	base: 'mt-4 min-w-0 border-l border-background-muted pl-4 text-sm text-foreground/75'
});
