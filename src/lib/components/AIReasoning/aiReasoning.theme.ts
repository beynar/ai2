...
const defaultTrigger = cva({
	base: 'flex w-fit max-w-full min-w-0 items-center justify-start gap-2 text-sm text-neutral/65 transition-colors hover:text-neutral'
});
...
const defaultContent = cva({
	base: 'mt-4 min-w-0 border-l border-neutral-muted pl-4 text-sm text-neutral/75'
});
