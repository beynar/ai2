...
const defaultAccordionTrigger = cva({
	base: 'min-h-8 max-w-full items-center gap-2 rounded-md text-left text-xs text-foreground/65 hover:bg-background-light hover:text-foreground',
	variants: {
		scope: {
			single: 'px-2 py-1.5',
			group: 'w-fit border border-background-muted bg-background-light/50 px-2.5 py-1.5',
			calls: 'px-2 py-1.5'
		}
	}
});
...
const defaultAccordionIcon = cva({ base: 'size-3 text-foreground/50' });
...
const defaultIndicator = cva({
	...
	variants: {
		tone: {
			...
			cancelled: 'border-foreground/20 bg-background-muted text-foreground/55',
			default: 'border-background-muted bg-background-light text-foreground/55'
		}
	}
});
const defaultIndicatorDot = cva({ base: 'size-1.5 rounded-full bg-current' });
const defaultGroupIcon = cva({
	base: 'flex size-5 shrink-0 items-center justify-center rounded-md border border-background-muted bg-background text-foreground/55'
});
const defaultName = cva({ base: 'min-w-0 flex-1 truncate font-medium text-foreground/85' });
...
const defaultSection = cva({
	base: 'grid min-w-0 gap-1.5 rounded-md border bg-background p-2',
	variants: {
		tone: {
			default: 'border-background-muted',
...
const defaultScrollArea = cva({
	...
	variants: {
		tone: {
			default: 'bg-background-light/60',
			error: 'bg-danger/5'
		}
	}
});
...
const defaultEmpty = cva({ base: 'py-1 text-xs text-foreground/55' });
...
const defaultTreeKey = cva({ base: 'min-w-0 truncate text-foreground/60' });
const defaultTreeSummary = cva({ base: 'shrink-0 text-[0.625rem] text-foreground/40' });
const defaultTreeChildren = cva({
	base: 'mt-1 ml-1 grid gap-1 border-l border-background-muted pl-3'
});
...
const defaultTreeValue = cva({
	...
	variants: {
		kind: {
			...
			null: 'text-foreground/50 italic',
			undefined: 'text-foreground/50 italic',
			empty: 'text-foreground/50 italic',
