...
const defaultMermaidRoot = cva({
	base: 'group/mermaid relative w-full overflow-hidden rounded border border-background-muted bg-background-light data-[expanded=true]:fixed data-[expanded=true]:inset-4 data-[expanded=true]:z-[2147483647] data-[expanded=true]:m-0 data-[expanded=true]:h-[calc(100vh_-_2rem)] data-[expanded=true]:w-[calc(100vw_-_2rem)] data-[expanded=true]:min-h-0 data-[expanded=true]:rounded-lg data-[expanded=true]:shadow-2xl',
	variants: {
		size: {
			small: 'h-64 min-h-48',
...
const defaultMermaidButtons = cva({
	base: 'absolute right-2 top-2 z-10 flex items-center gap-1 rounded-md border border-background-muted bg-background/80 p-1 opacity-0 backdrop-blur transition-opacity group-hover/mermaid:opacity-100 focus-within:opacity-100 data-[expanded=true]:opacity-100'
});

const defaultMermaidError = cva({
	base: 'absolute inset-0 z-20 flex items-center justify-center overflow-auto p-6 text-center text-sm rounded border border-danger/40 bg-danger-muted text-danger'
});
