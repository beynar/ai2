...
AIComposer is a Markdown prompt composer built on the existing \`RichTextInput\`, file-acceptance helpers, \`SortableList\`, \`ScrollArea\`, and \`AIFilePreview\`. It adds AI command, mention, reference, and skill tokens; raw or managed files; queue editing; steering; submit/stop state; and optional \`AIConversation\` integration without owning transport.
...
\`header\`, \`prefix\`, \`suffix\`, and \`modelSelector\` are content slots. \`dropzone\` customizes the active file-drop overlay. \`footerStart\`, \`actions\`, and replacement \`footer\` receive \`{ value, files, attachments, isEmpty, isBusy }\`.
...
The root accepts native form attributes plus bindable \`ref\`, \`class\`, and \`theme\`. Theme parts cover root, dropzone/dropzone icon, header, files, file rows, body, editor, toolbar, errors, footer/actions, and every queue region.
`;
