...
- A data mark can own \`arrow\`, \`label\`, \`rule\`, \`band\`, and \`marker\` annotations. Targets resolve against the parent mark's final rendered coordinates.
- Position scales use local string discriminants such as \`linear\`, \`utc\`, and \`band\`.
- \`tooltip: true\` groups cartesian marks on the categorical or x axis, renders native color rows, and uses a viewport portal without adding chart focus states.
- Tooltip objects can override grouping with \`groupBy: 'x'\`, \`groupBy: 'y'\`, or \`groupBy: false\`.
- \`viewport: true\` enables x-axis brush zoom, a reset control, and a reduced-motion-aware transition. Use \`viewport: { axis: 'both' }\` for a rectangular x/y selection such as a scatter-plot zoom. The object form also accepts \`drag: 'brush-zoom'\`, \`reset\`, and \`transition\` options.
