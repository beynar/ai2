...
describe('Chart in the browser', () => {
	...
	test('keeps keyboard focus and the native tooltip active', async () => {
		const { container } = render(RevenueChart, {
			props: { data, ...definition, ariaLabel: 'Monthly revenue' }
		});
	...
	test('groups series at the focused x value in a portaled tooltip', async () => {
		...
		await waitFor(() => {
			...
			expect(tooltip).toHaveAttribute('data-ts-chart-tooltip-portal', 'fallback');
	...
	test('removes chart DOM during cleanup', async () => {
		...
		(container.querySelector('svg') as SVGElement).focus();
		await waitFor(() => expect(document.querySelector('[role="status"]')).toBeInTheDocument());
		unmount();
