...
const defaultTriggerIcon = cva({
	base: 'flex size-5 shrink-0 items-center justify-center rounded-sm border border-background-muted bg-background text-xs'
});
const defaultTriggerLabel = cva({
	base: 'min-w-0 truncate',
	variants: { selected: { true: 'text-foreground', false: 'text-foreground/60' } },
	defaultVariants: { selected: false }
});
const defaultSearch = cva({ base: 'p-1' });
const defaultPopover = cva({ base: 'min-w-56' });
const defaultMenu = cva({ base: 'min-w-56' });
const defaultProvider = cva({ base: 'justify-start text-xs font-semibold text-foreground/55' });
...
const defaultEmpty = cva({ base: 'justify-start text-foreground/55' });
