...
const defaultRoot = cva({
	base: 'raised grid min-w-0 gap-3 rounded-lg bg-surface-raised p-3 text-sm text-neutral'
});
...
const defaultRequester = cva({
	base: 'break-words text-[0.6875rem] text-neutral/55 [overflow-wrap:anywhere]'
});
...
const defaultBody = cva({
	base: 'grid min-w-0 gap-3 rounded-md border border-neutral-muted bg-surface p-3'
});
...
const defaultQuestionTitle = cva({
	base: 'break-words font-medium leading-snug text-neutral [overflow-wrap:anywhere]'
});
